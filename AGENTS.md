...
- Dictionary lemma images: `bun run images:fetch` (SK/EN Wikipedia free pageimages, then Commons gloss search for Food / Places / People / … → `static/images/dictionary/`, gitignored; manifest in `content/images/`) then `bun run images:upload` (R2, keys `images/dictionary/…`). Nouns / adjectives / verbs: stage → promote only. Person names are not dictionary entries (no lemmas / audio / images). Local plays `/images/dictionary/…`; production needs `PUBLIC_IMAGE_BASE_URL`. Overrides: `content/images/overrides.json`. Coverage: `bun run images:status`. See `scripts/README.md` `images/`.
