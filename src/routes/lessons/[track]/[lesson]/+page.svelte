...
<svelte:head><title>{data.lesson.title} | Slovak Wiki Lessons</title><meta name="description" content={data.lesson.promise}></svelte:head>

<main class="px-[30px] py-12 pb-[78px] max-[640px]:px-3.5 max-[640px]:py-8 max-[640px]:pb-[50px]"><article class="mx-auto w-full max-w-[790px]"><nav class="mb-6 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb"><a class="text-blue-800 underline underline-offset-2" href="/lessons">Lessons</a><span>/</span><a class="text-blue-800 underline underline-offset-2" href="/lessons/{data.lesson.track}">{data.lesson.track}</a></nav>
  <header class="border-b border-slate-200 py-6 pb-8"><p class="section-label">{data.lesson.track}</p><h1>{data.lesson.title}</h1><p class="lead">{data.lesson.promise}</p></header>
  ...
  <section class="scroll-mt-[72px] mt-7 border-t border-slate-200 pt-8" aria-labelledby="phrases-heading"><p class="section-label">Keep these close</p><h2 id="phrases-heading" class="mb-4 text-2xl">Key phrases</h2><KeyPhraseList phrases={data.lesson.keyPhrases} /></section>
