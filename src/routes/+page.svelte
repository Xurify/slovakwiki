<script lang="ts">
  ...
  let query = $state("");
  function handleSubmit(event: SubmitEvent): void { event.preventDefault(); const trimmedQuery = query.trim(); if (trimmedQuery) goto(`/search?q=${encodeURIComponent(trimmedQuery)}`); }
...
<svelte:head><title>Slovak Wiki — Slovak language reference and practice</title><meta name="description" content="A practical Slovak dictionary, grammar reference, pronunciation guide, lessons, and focused practice for English speakers."></svelte:head>
...
<main class="home-page">
  <section class="border-b border-slate-200 bg-slate-50">
    <div class="shell grid grid-cols-[minmax(320px,.88fr)_minmax(420px,1.12fr)] items-center gap-16 py-12 max-[900px]:grid-cols-1 max-[900px]:gap-7 max-[600px]:py-8">
      ...
      <form class="rounded-lg bg-slate-800 p-6 text-white shadow-lg max-[600px]:p-4" role="search" onsubmit={handleSubmit}>
        <label class="mb-2 block text-sm font-semibold" for="home-search">Search the reference</label>
        <div class="flex min-h-[50px] rounded border border-white/30 bg-white focus-within:border-white focus-within:ring-4 focus-within:ring-white/20 max-[600px]:flex-col"><input class="min-w-0 flex-1 border-0 bg-transparent px-3.5 text-slate-900 outline-0 max-[600px]:min-h-[46px]" id="home-search" bind:value={query} type="search" placeholder="Try ďakujem, cases, or soft consonants"><button class="min-w-[92px] cursor-pointer rounded-r bg-rose-600 px-4 font-bold text-white hover:bg-rose-700 max-[600px]:min-h-[46px] max-[600px]:rounded-b max-[600px]:rounded-r-none" type="submit">Search</button></div>
