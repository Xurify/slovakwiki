...
<main class="reference-page">
  <header class="page-header"><div class="shell"><p class="breadcrumb">Reference / Grammar</p><h1>Grammar</h1><p>Rules and patterns for understanding Slovak sentences. <a href="/grammar/terms">Language terms →</a></p></div></header>
  <section class="shell index" aria-label="Grammar topics">
    {#each groups as group (group)}
      {@const topics = grammarEntries.filter((topic) => topic.pathGroup === group)}
      {#if topics.length}
        <section><h2>{group}</h2><div class="topic-list">{#each topics as topic (topic.slug)}<a href="/grammar/{topic.slug}"><strong>{topic.english}</strong><span lang="sk">{topic.slovak}</span><p>{topic.summary}</p><i aria-hidden="true">›</i></a>{/each}</div></section>
      {/if}
    {/each}
  </section>
</main>
...
<style>
  .page-header { padding:28px 0 25px; border-bottom:1px solid var(--line); background:color-mix(in srgb,var(--surface-subtle) 40%,transparent); } .breadcrumb { margin:0 0 16px; color:var(--accent); font-size:.7rem; font-weight:750; letter-spacing:.06em; text-transform:uppercase; } h1 { font-size:clamp(2.45rem,5vw,4rem); } .page-header p:last-child { max-width:590px; margin:10px 0 0; color:var(--ink-soft); font-family:var(--font-reading); font-size:1rem; } .page-header p:last-child a { color:var(--accent-dark); text-decoration:underline; text-underline-offset:3px; }
