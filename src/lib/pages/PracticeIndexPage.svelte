...
<main class={cx(shell, page, "max-w-[960px]", "pt-14", "max-[620px]:pt-8")}>
  ...
  >
    <div>
      ...
      <span class="mt-2 block max-w-[50ch] font-serif leading-6 text-slate-700"
        >Missed and revealed forms return here. Nothing else is tracked.</span
      ><small class="mt-3 block text-xs text-slate-500"
        ><b class="mr-1 font-serif text-sm text-blue-800" lang="sk">Skús to znova.</b> Try it again.</small
      >
    </div>
  ...
  <section class="mt-9 max-w-[780px]" aria-labelledby="topics-heading">
    <div class="mb-3.5">
      ...
      <h2 id="topics-heading" class="text-2xl font-semibold text-slate-900">From your lessons</h2>
    </div>
    {#if !hydrated || completedSets.length}
      <div class="border-t border-slate-200">
        {#each completedSets as set (set.id)}<a
            ...
            ><span
              ...
              >{set.track}</span
            ><strong class="font-serif text-lg text-blue-800">{set.title}</strong><small
              class="text-xs text-slate-600">Practice again <b aria-hidden="true">→</b></small
            ></a
          >{/each}
      </div>
    {:else}
  ...
  {#if hydrated && practiceState.savedReferenceItemIds.length}
    ...
    >
      ...
      <h2 id="saved-heading" class="text-2xl font-semibold text-slate-900">Focused items</h2>
      <div class="mt-3.5 border-t border-slate-200">
        {#each savedItems as item (item.id)}<a
            ...
            ><span
              class="text-xs font-bold uppercase tracking-wide text-slate-500 max-[620px]:col-span-2"
              >{item.source.label}</span
            ><strong class="font-serif text-blue-800">{item.task.prompt}</strong><b
              class="text-blue-800"
              aria-hidden="true">→</b
            ></a
          >{/each}
