...
<main class={cx(shell, page, "max-w-[960px]", "pt-14", "max-[620px]:pt-8")}>
  <header class="max-w-[690px]"><p class={sectionLabel}>Practice</p><h1>Bring Slovak back.</h1><p class={lead}>Review what was uncertain, or return to a lesson topic when you want another pass.</p></header>
  ...
  <section class="mt-9 max-w-[780px]" aria-labelledby="topics-heading">
    ...
    {:else}
      <div class="border-y border-slate-200 py-5"><p class="m-0 font-serif text-slate-700">Finish a lesson to practise its language again here.</p><a class={textLink} href="/lessons">Browse lessons</a></div>
    {/if}
  </section>
  ...
  {#if hydrated && practiceState.savedReferenceItemIds.length}
    <section class="mt-9 max-w-[780px] border-t border-slate-200 pt-7" aria-labelledby="saved-heading">
      <p class={sectionLabel}>Saved from Reference</p><h2 id="saved-heading" class="text-2xl font-semibold text-slate-900">Focused items</h2>
