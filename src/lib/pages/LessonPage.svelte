<script lang="ts">
  ...
  const trackTitle = $derived(
    lessonTracks.find((track) => track.id === data.lesson.track)?.title ?? data.lesson.track,
  );
  ...
  function resolveExercise(result: { needsReview: boolean; practiceItemId?: string }): void {
...
<main class="px-[30px] py-12 pb-[78px] max-[640px]:px-3.5 max-[640px]:py-8 max-[640px]:pb-[50px]">
  <article class="mx-auto w-full max-w-[790px]">
    <nav class="mb-6 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb">
      <a class="text-blue-800 underline underline-offset-2" href="/lessons">Lessons</a><span>/</span
      ><a class="text-blue-800 underline underline-offset-2" href="/lessons/{data.lesson.track}"
        >{trackTitle}</a
      >
    </nav>
    ...
    {#if data.lesson.pattern}<section class="scroll-mt-[72px] mt-7 border-t border-slate-200 pt-8">
    ...
    >
      ...
      {#if !hydrated}<p class="font-serif text-slate-500">
          Loading your lesson…
        </p>{:else if finished}<div class="border-l-4 border-emerald-600 bg-emerald-50 p-6">
          <p class="m-0 text-xs font-semibold uppercase tracking-widest text-emerald-700">
            Lesson complete
          </p>
          <h3 class="mb-1 mt-2 font-serif text-2xl text-slate-900">Keep the scene, not a score.</h3>
          <span class="font-serif text-slate-700"
            >Anything you missed or revealed is ready in Review.</span
          >
          <div class="mt-5 flex flex-wrap items-center gap-4">
            <a class={cx(button, "min-h-10")} href="/practice/review">Open Review</a><a
              class={textLink}
              href="/lessons">Browse lessons</a
            >
          </div>
        </div>{:else}<p class="m-0 mb-2 text-xs text-slate-500">
          Step {activeIndex + 1} of {data.lesson.exercises.length}
        </p>
        {#key currentExercise.id}<LessonInteraction
    ...
    <footer class="mt-12 border-t border-slate-200 pt-8">
      ...
      <div class="grid grid-cols-[repeat(auto-fit,minmax(170px,1fr))] gap-2">
        {#each data.lesson.referenceLinks as link (link.href)}<a
            class="group flex min-h-16 items-center justify-between gap-4 border border-slate-200 bg-slate-50 px-4 py-3.5 font-serif text-base text-blue-800 after:font-sans after:text-lg after:text-slate-500 after:transition after:duration-150 after:ease-out after:content-['→'] hover:border-slate-300 hover:bg-blue-50 group-hover:after:translate-x-[0.15rem] group-hover:after:text-blue-800"
