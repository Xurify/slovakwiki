...
<main>
  <header class="border-b border-slate-200 bg-slate-50 py-8">
    <div class={shell}>
      <nav class="mb-5 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb">
        <a class="text-blue-700 underline underline-offset-2" href="/wiki">Reference index</a><span
          >/</span
        ><span>Search</span>
      </nav>
  ...
  <section class={cx(shell, "py-8", "pb-16")} aria-labelledby="results-heading">
    <div class="grid grid-cols-[minmax(0,1fr)_210px] gap-8 max-[760px]:grid-cols-1">
      <div>
        {#if results.length}
          <div class="flex items-baseline justify-between gap-5 border-b border-slate-300 pb-3">
            <h2 id="results-heading" class="text-xl font-semibold text-slate-900">Matches</h2>
            <a class="text-xs font-bold text-blue-800 underline underline-offset-2" href="/wiki"
              >Complete index</a
            >
          </div>
          <div class="border-b border-slate-200">
            {#each results as entry (entry.slug)}<EntryCard {entry} />{/each}
          </div>
        {:else}
      ...
      >
        ...
        <section class="mt-7">
          ...
          <strong class="block truncate font-serif text-blue-800">{query || "None"}</strong><span
            class="mt-0.5 block text-xs text-slate-500"
            >{results.length} {results.length === 1 ? "match" : "matches"}</span
          >
        </section>
        <section class="mt-7">
          <p class="mb-2 text-xs font-semibold uppercase tracking-widest text-blue-700">
          ...
          <a class="font-serif text-sm text-blue-800 underline underline-offset-2" href="/wiki"
            >Open Slovak Wiki</a
          >
        </section>
      </aside>
