...
<main class={cx(shell, page, "max-w-[900px]", "pt-12")}>
  <nav class="mb-7 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb">
    <a class="text-blue-800 underline underline-offset-2" href="/lessons">Lessons</a><span
  ...
  <section aria-labelledby="lesson-list-heading">
    ...
    <ol class="m-0 list-none p-0">
      {#each data.lessons as lesson, index (lesson.id)}<li class="border-b border-slate-200">
          <a
            ...
            <div class="grid gap-1">
              {#if lesson.group}<small
                  class="text-xs font-bold uppercase tracking-wide text-slate-500"
                  >{lesson.group}</small
                >{/if}<strong class="font-serif text-xl text-blue-800">{lesson.title}</strong>
              <p class="m-0 max-w-[56ch] font-serif leading-6 text-slate-700">{lesson.promise}</p>
            </div>
            <b class="text-lg text-blue-800" aria-hidden="true">→</b></a
          >
        </li>{/each}
    </ol>
  </section>
