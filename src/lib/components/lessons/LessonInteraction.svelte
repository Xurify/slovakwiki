<script lang="ts">
  ...
  function continueLesson(): void {
    if (graded)
      onresolve({ needsReview: revealed || !correct, practiceItemId: graded.practiceItemId });
  }
  ...
  {:else}
    ...
    {:else if exercise.type === "build"}
      <div class="mt-6 grid gap-3" aria-label="Build the sentence">
        ...
        <div class="flex flex-wrap gap-2">
          {#each exercise.tiles as tile, index (`${tile}-${index}`)}<button
              ...
              onclick={() => addTile(tile)}>{tile}</button
            >{/each}
        </div>
      </div>
    {:else}
      <label class="mt-6 grid gap-2 text-xs font-bold text-slate-600"
        ><span>{exercise.inputLabel}</span><input
    ...
    {#if submitted}
      ...
      >
        ...
        {#if exercise.feedback.correction}<strong
            class="font-serif text-lg text-slate-900"
            lang="sk">{exercise.feedback.correction}</strong
          >{/if}{#if exercise.feedback.english}<small class="text-slate-500"
            >{exercise.feedback.english}</small
          >{/if}<span class="mt-1 max-w-[67ch] font-serif leading-6 text-slate-700"
          >{exercise.feedback.why}</span
        >
      </div>
      <button class={cx(button, "mt-6")} type="button" onclick={continueLesson}>Continue</button>
    {:else}
      <div class="mt-6 flex items-center gap-4 max-[560px]:flex-col max-[560px]:items-stretch">
        <button
          class={cx(button, "mt-0", "max-[560px]:w-full")}
          type="button"
          disabled={!canCheck}
          onclick={check}>Check</button
        >{#if exercise.type === "typed"}<button
