<script lang="ts">
  ...
  let { exercise, onresolve }: { exercise: LessonExercise; onresolve: (result: { needsReview: boolean; practiceItemId?: string }) => void } = $props();
  let selectedId = $state<string | null>(null); let builtTiles = $state<string[]>([]); let input = $state(""); let submitted = $state(false); let revealed = $state(false); let feedbackPanel = $state<HTMLElement | null>(null);
  const graded = $derived(exercise.type !== "personal" ? exercise : null);
  const correct = $derived.by(() => { if (!graded || !submitted || revealed) return false; if (graded.type === "choice") return selectedId === graded.answerId; if (graded.type === "build") return builtTiles.length === graded.answer.length && builtTiles.every((tile, index) => tile === graded.answer[index]); return answersMatch(input, graded.answer, graded.acceptedAnswers); });
  const canCheck = $derived.by(() => { if (!graded || submitted) return false; if (graded.type === "choice") return selectedId !== null; if (graded.type === "build") return builtTiles.length === graded.tiles.length; return input.trim().length > 0; });
  async function focusFeedback(): Promise<void> { await tick(); feedbackPanel?.focus(); }
  async function check(): Promise<void> { if (!canCheck || !graded) return; submitted = true; await focusFeedback(); }
  async function reveal(): Promise<void> { if (!graded || submitted) return; revealed = true; submitted = true; await focusFeedback(); }
  function continueLesson(): void { if (graded) onresolve({ needsReview: revealed || !correct, practiceItemId: graded.practiceItemId }); }
  function addTile(tile: string): void { if (!submitted) builtTiles = [...builtTiles, tile]; }
...
<section class={cx(surfacePanel, "bg-white", "p-7", "max-[560px]:px-4", "max-[560px]:py-5")} aria-labelledby="interaction-heading">
  ...
  {:else}
    ...
    {:else if exercise.type === "build"}
      <div class="mt-6 grid gap-3" aria-label="Build the sentence"><div class="flex min-h-[60px] flex-wrap content-center gap-2 border border-slate-300 bg-slate-50 p-3" aria-live="polite">{#if builtTiles.length}{#each builtTiles as tile, index (`${tile}-${index}`)}<button class="cursor-pointer border border-slate-300 bg-white px-2.5 py-1.5 font-serif font-semibold text-blue-800 disabled:cursor-default" type="button" disabled={submitted} onclick={() => removeTile(index)}>{tile}</button>{/each}{:else}<span class="text-sm text-slate-500">Choose the words in order.</span>{/if}</div><div class="flex flex-wrap gap-2">{#each exercise.tiles as tile, index (`${tile}-${index}`)}<button class="cursor-pointer border border-slate-300 bg-white px-3 py-2 font-serif font-semibold text-blue-800 hover:border-blue-600 hover:bg-blue-50 disabled:cursor-default disabled:opacity-40" type="button" disabled={submitted || builtTiles.includes(tile)} onclick={() => addTile(tile)}>{tile}</button>{/each}</div></div>
    {:else}
