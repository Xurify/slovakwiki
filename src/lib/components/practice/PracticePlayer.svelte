...
{#if finished}
  ...
  >
    <p class="m-0 text-xs font-semibold uppercase tracking-widest text-emerald-700">Finished</p>
    <h2 id="practice-finished-heading" class="mb-1 mt-2 text-3xl">Keep the useful ones close.</h2>
...
{:else}
  <section class="max-w-[720px]" aria-labelledby="practice-question">
    ...
    >
      <span>{mode === "review" ? "Review" : "Practice"}</span><span
        >{activeIndex + 1} of {items.length}</span
      >
    </div>
    ...
    {#if task.context?.length}<div class="mt-8 grid border-t border-slate-200">
        {#each task.context as line (line.id)}<div
          ...
          >
            <span class="text-xs font-bold uppercase text-blue-700">{line.speaker}</span><strong
              class="font-serif text-base"
              lang="sk">{line.slovak}</strong
            ><AudioButton
              src={line.audio?.src}
    ...
    {#if task.type === "choice"}<div class="mt-7 grid gap-2">
        {#each task.choices as choice (choice.id)}<button
            ...
            onclick={() => (selectedId = choice.id)}>{choice.label}</button
          >{/each}
      </div>
    {:else if task.type === "build"}<div class="mt-7 grid gap-3">
        <div
          class="flex min-h-[62px] flex-wrap content-center gap-2 border border-slate-300 bg-slate-50 p-3"
          aria-live="polite"
        >
          {#if builtTiles.length}{#each builtTiles as tile, index (`${tile}-${index}`)}<button
                class="border border-slate-300 bg-white px-2.5 py-2 font-serif font-semibold text-blue-800"
                type="button"
                disabled={submitted}
                onclick={() => removeTile(index)}>{tile}</button
              >{/each}{:else}<span class="text-sm text-slate-500">Choose the words in order.</span
            >{/if}
        </div>
        <div class="flex flex-wrap gap-2">
          {#each task.tiles as tile, index (`${tile}-${index}`)}<button
              ...
              onclick={() => selectTile(tile)}>{tile}</button
            >{/each}
        </div>
      </div>
    {:else}<label class="mt-7 grid gap-2 text-xs font-bold text-slate-600"
        ><span>{task.inputLabel}</span><input
    ...
    {#if submitted}<div
      ...
      >
        ...
        >{#if current.feedback.english}<small class="text-slate-500"
            >{current.feedback.english}</small
          >{/if}<span class="max-w-[65ch] font-serif leading-6 text-slate-700"
          >{current.feedback.why}</span
        >{#if current.newUse}<em class="mt-1 font-serif not-italic text-blue-800"
            >{current.newUse}</em
          >{/if}
      </div>
      <button class={cx(button, "mt-5")} type="button" onclick={next}
        >{activeIndex === items.length - 1 ? "Finish" : "Continue"}</button
      >{:else}<div
