    ...
    >
      <header class="border-b border-slate-200 pb-7">
        <nav class="mb-5 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb">
          <a class="text-blue-800 underline underline-offset-2" href="/wiki">Reference</a><span
            >/</span
          ><span>{kindLabel[entry.kind]}</span>
        </nav>
        <h1 lang="sk">{entry.slovak}</h1>
        <div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-slate-500">
          <span class="h-2 w-2 rounded-full bg-blue-600 ring-4 ring-blue-100" aria-hidden="true"
          ></span><strong class="font-serif text-base text-blue-800">{entry.english}</strong><span
            >{entry.category}</span
          >
        </div>
      ...
      <section id="usage" class="scroll-mt-[72px] pt-8" aria-labelledby="usage-heading">
        ...
        {#each entry.body as paragraph, index (index)}<p
            class="max-w-[67ch] font-serif leading-7 text-slate-700"
          >
            {paragraph}
          </p>{/each}
      </section>
      ...
      >
        ...
        <ol class={cx(surfacePanel, "mt-5", "m-0", "list-none", "rounded", "bg-slate-50", "p-0")}>
          {#each entry.examples as example, index (`${example.slovak}-${index}`)}<li
      ...
      >
        ...
        <a
          ...
          href={entry.source}>Jazykovedný ústav Ľudovíta Štúra SAV ↗</a
        >
      </section>
    </article>
    <aside
    ...
    >
      ...
      <section>
        ...
        <ul class="m-0 list-none p-0">
          {#each entry.related as relatedSlug (relatedSlug)}{@const relatedEntry =
              entryBySlug.get(relatedSlug)}{#if relatedEntry}<li class="border-b border-slate-200">
                <a
                  class="grid gap-0.5 py-2"
                  href="/{routeBase[relatedEntry.kind]}/{relatedEntry.slug}"
                  ><strong class="font-serif text-sm text-blue-800">{relatedEntry.slovak}</strong
                  ><small class="truncate text-xs text-slate-500">{relatedEntry.english}</small></a
                >
              </li>{/if}{/each}
        </ul>
      </section>
