<script lang="ts">
  import { contextRail, cx, railLabel, sectionLabel, surfacePanel } from "$lib/ui/classes";
...
<main><div class="grid grid-cols-[minmax(0,1fr)_210px] max-[1080px]:block">
  <article class="mx-auto w-full max-w-[760px] px-[30px] py-[34px] pb-[74px] max-[760px]:px-3.5 max-[760px]:py-7 max-[760px]:pb-[52px]">
    ...
    <section id="usage" class="scroll-mt-[72px] pt-8" aria-labelledby="usage-heading"><p class={sectionLabel}>Usage</p><h2 id="usage-heading" class="mb-3 text-2xl">How to use it</h2>{#each entry.body as paragraph, index (index)}<p class="max-w-[67ch] font-serif leading-7 text-slate-700">{paragraph}</p>{/each}</section>
    <section id="examples" class="scroll-mt-[72px] mt-6 border-t border-slate-200 pt-8" aria-labelledby="examples-heading"><p class={sectionLabel}>Examples</p><h2 id="examples-heading" class="mb-3 text-2xl">In a sentence</h2><ol class={cx(surfacePanel, "mt-5", "m-0", "list-none", "rounded", "bg-slate-50", "p-0")}>{#each entry.examples as example, index (`${example.slovak}-${index}`)}<li class="grid grid-cols-[32px_1fr] gap-3 border-b border-slate-200 px-4 py-3.5 last:border-b-0"><span class="text-xs font-bold text-blue-700">{String(index + 1).padStart(2, "0")}</span><div><p class="m-0 mb-0.5 font-serif font-semibold text-slate-900" lang="sk">{example.slovak}</p><small class="text-xs text-slate-500">{example.english}</small></div></li>{/each}</ol></section>
    <section id="source" class="scroll-mt-[72px] mt-6 border-t border-slate-200 pt-8" aria-labelledby="source-heading"><p class={sectionLabel}>Source</p><h2 id="source-heading" class="mb-3 text-2xl">Reference</h2><a class="font-serif font-semibold text-blue-800 underline underline-offset-2" href={entry.source}>Jazykovedný ústav Ľudovíta Štúra SAV ↗</a></section>
  </article>
  <aside class={cx(contextRail, "sticky", "top-(--header-height)", "h-fit", "min-w-0", "border-l", "border-slate-200", "px-[18px]", "pb-12", "pt-8", "max-[1080px]:static", "max-[1080px]:grid", "max-[1080px]:grid-cols-2", "max-[1080px]:gap-8", "max-[1080px]:border-l-0", "max-[1080px]:border-t", "max-[1080px]:px-[30px]", "max-[1080px]:py-6", "max-[760px]:grid-cols-1", "max-[760px]:gap-6", "max-[760px]:px-3.5")} aria-label="Entry navigation">
    <section><p class={railLabel}>On this page</p><nav class="grid"><a class="border-l-2 border-blue-600 py-1.5 pl-2.5 font-serif text-sm text-blue-800" href="#usage">How to use it</a><a class="border-l-2 border-slate-200 py-1.5 pl-2.5 font-serif text-sm text-slate-700 hover:border-blue-600 hover:text-blue-800 hover:underline" href="#examples">Examples</a><a class="border-l-2 border-slate-200 py-1.5 pl-2.5 font-serif text-sm text-slate-700 hover:border-blue-600 hover:text-blue-800 hover:underline" href="#source">Reference</a></nav></section>
    <section><p class={railLabel}>Related entries</p><ul class="m-0 list-none p-0">{#each entry.related as relatedSlug (relatedSlug)}{@const relatedEntry = entryBySlug.get(relatedSlug)}{#if relatedEntry}<li class="border-b border-slate-200"><a class="grid gap-0.5 py-2" href="/{routeBase[relatedEntry.kind]}/{relatedEntry.slug}"><strong class="font-serif text-sm text-blue-800">{relatedEntry.slovak}</strong><small class="truncate text-xs text-slate-500">{relatedEntry.english}</small></a></li>{/if}{/each}</ul></section>
  </aside>
</div></main>
