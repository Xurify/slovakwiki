...
>
  <article class="min-w-0">
    ...
    {#if topic.lessonLink}<aside class="mt-3 border-l-4 border-emerald-600 bg-emerald-50 px-4 py-3">
        ...
        <a
          ...
          href={topic.lessonLink.href}>{topic.lessonLink.label} <span aria-hidden="true">→</span></a
        >
      </aside>{/if}
    ...
    <section class="scroll-mt-[72px] pt-8">
      ...
      {#each topic.rule as paragraph (paragraph)}<p
          class="max-w-[66ch] font-serif leading-7 text-slate-700"
        >
          {paragraph}
        </p>{/each}
    </section>
    {#if !topic.caseOverview}<section class="scroll-mt-[72px] border-t border-slate-200 pt-8">
        ...
        <ul class="mt-5 m-0 list-none rounded border border-slate-200 p-0">
          {#each topic.pattern.lines as line (line)}<li
    ...
    {#if topic.caseOverview}<section
      ...
      >
        ...
        <p class="mb-4 font-serif text-slate-500">
          Learn the nominative first. Use the remaining rows as a map of common roles, then open
          each case for examples.
        </p>
        <ol class="m-0 grid list-none grid-cols-2 gap-2 p-0 max-[560px]:grid-cols-1">
          {#each topic.caseOverview as item (item.name)}<li
            ...
            >
              <a class="grid min-h-[102px] gap-1 p-3" href="/grammar/cases/{item.slug}"
                ><div class="flex justify-between gap-2">
                  <strong class="font-serif text-blue-800 hover:underline">{item.name}</strong
                  >{#if item.role}<span class="text-xs text-slate-500">{item.role}</span>{/if}
                </div>
                {#if item.question}<p class="m-0 text-xs text-slate-500">
                    {item.question}
                  </p>{/if}{#if item.explanation}<small
                    class="font-serif text-sm leading-5 text-slate-700">{item.explanation}</small
                  >{/if}{#if item.researchPrompt}<small class="text-xs italic text-slate-500"
                    >Research: {item.researchPrompt}</small
                  >{/if}</a
              >
            </li>{/each}
        </ol>
    ...
    {#if !topic.caseOverview}<section class="scroll-mt-[72px] border-t border-slate-200 pt-8">
        ...
        <ol class="mt-5 grid list-none gap-2 p-0">
          {#each topic.examples as example (example.slovak)}<li
              class="grid gap-1 border-l-4 border-blue-600 bg-slate-50 px-4 py-3"
            >
              <strong class="font-serif" lang="sk">{example.slovak}</strong><span
                class="text-sm text-slate-500">{example.english}</span
              >{#if example.demonstrates}<small
                  class="max-w-[60ch] font-serif text-sm leading-5 text-slate-700"
                  >What this shows: {example.demonstrates}</small
                >{/if}{#if example.practiceItemId}<FocusedPracticeAction
                  itemId={example.practiceItemId}
                />{/if}
            </li>{/each}
        </ol>
      </section>{/if}
    <aside class="mt-7 border border-slate-300 bg-blue-50 p-4">
      ...
      <p class="mb-0 max-w-[66ch] font-serif leading-7 text-slate-700">{topic.watchOut}</p>
    </aside>
  </article>
  <aside
  ...
  >
    <section>
      ...
      <a
        ...
        href="#rule-heading">Core rule</a
      >{#if topic.caseOverview}<a
          class="block py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#case-map">Case map</a
        >{:else}<a
          class="block py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#pattern-heading">Pattern</a
        ><a
          class="block py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#examples-heading">Examples</a
        >{/if}
    </section>
    {#if relatedWords.length}<section>
        <p class={railLabel}>Words to know</p>
        {#each relatedWords as word (word.slug)}<a
            class="grid gap-0.5 py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
            href="/dictionary/{word.slug}"
            lang="sk">{word.slovak}<small class="text-xs text-slate-500">{word.english}</small></a
          >{/each}
      </section>{/if}{#if relatedTopics.length}<section>
        <p class={railLabel}>Related topics</p>
        {#each relatedTopics as entry (entry.slug)}<a
            class="grid gap-0.5 py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
            href="/{routeBase[entry.kind]}/{entry.slug}"
            >{entry.english}<small class="text-xs text-slate-500">{entry.slovak}</small></a
          >{/each}
      </section>{/if}
  </aside>
</main>
