...
>
  <article class="min-w-0">
    <nav class="mb-5 flex gap-2 text-xs text-slate-500" aria-label="Breadcrumb">
      <a class="text-blue-800 underline underline-offset-2" href="/pronunciation">Pronunciation</a
      ><span>/</span><span>{topic.pathGroup}</span>
    </nav>
    ...
    <section class="scroll-mt-[72px] border-t border-slate-200 pt-8">
      ...
      <div class="mt-5 grid grid-cols-2 gap-2.5 max-[560px]:grid-cols-1">
        {#each topic.contrasts as contrast (contrast.left)}<div
            class="grid gap-1 rounded border border-slate-200 bg-slate-50 p-3.5"
          >
            <strong class="font-serif text-blue-800" lang="sk"
              >{contrast.left} / {contrast.right}</strong
    ...
    <section class="scroll-mt-[72px] border-t border-slate-200 pt-8">
      <div class="rounded border border-slate-300 bg-blue-50 p-5">
        ...
        <p class="mb-0 max-w-[66ch] font-serif leading-7 text-slate-700">{topic.mouthCue}</p>
      </div>
    </section>
    <section class="scroll-mt-[72px] border-t border-slate-200 pt-8">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        {#each topic.practiceWords as word (word)}<span
            class="rounded-full border border-slate-300 px-2.5 py-1.5 font-serif text-blue-800"
            lang="sk">{word}</span
          >{/each}
      </div>
      <blockquote class="mt-4 grid gap-1 border-l-4 border-blue-600 bg-slate-50 px-4 py-3">
        <strong class="font-serif" lang="sk">{topic.practicePhrase.slovak}</strong><span
          class="text-sm text-slate-500">{topic.practicePhrase.english}</span
        >
      </blockquote>
    </section>
  </article>
    ...
    <section>
      ...
      <nav class="grid">
        <a
          ...
          href="#goal-heading">Hear and say</a
        ><a
          class="py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#contrast-heading">Contrasts</a
        ><a
          class="py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#cue-heading">Mouth cue</a
        ><a
          class="py-1.5 font-serif text-sm text-slate-700 hover:text-blue-800 hover:underline"
          href="#practice-heading">Practice</a
        >
      </nav>
    </section>
